...
async def add_welcome(guild_id, channel_id) -> int:
    """
    This function will add a warn to the database.
    ...
    """
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("INSERT INTO greetings(guild_id, channel_id) VALUES (?, ?)", (guild_id, channel_id))
        await db.commit()
        return guild_id, channel_id

async def get_welcome(guild_id:int):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        channel=await db.execute("SELECT channel_id FROM greeting WHERE guild_id = ?", (guild_id))
        return channel
