...
class Moderation(commands.Cog, name="moderation"):
    def __init__(self, bot):
        ...
        self.too_many_spam= commands.CooldownMapping.from_cooldown(4, 60, commands.BucketType.member)
    ...
    async def purge(self, context: Context, amount: int=None) -> None:
        """
        ...
        if amount==None:
	        purged_messages = await context.channel.purge(limit=50)
        	embed = discord.Embed(description=f"**{context.author}** cleared **{len(purged_messages)}** messages!", color=0x2f3136)
    ...
    async def hackban(self, context: Context, user_id: str, *, reason: str = "Not specified") -> None:
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"An error occurred while trying to ban the user. Make sure ID is an existing ID that belongs to a user.\n **Exception:**\n```{e}```",
                color=0x2f3136
            )
            await context.send(embed=embed)
    ...
    async def on_message(self, message):
        if message.author.id==self.bot.user.id:
	        return
        if has_permissions(manage_messages=True) and has_permissions(administrator=True):
            return
        msg_content=message.content.lower()
        banned=["https://", ".gg", "https://www.", "tenor", ".com", "discord.com/invite", "discord.gg"]
        if any(word in msg_content for word in banned):
            await message.delete()
            await message.channel.send(embed=discord.Embed(description=f"hey {message.author.mention}, don't send links", color=0x2f3136), delete_after=10)
        ...
        if retry_after:
            await message.delete()
            await message.channel.send(embed=discord.Embed(description=f"hey {message.author.mention}, don't spam", color=0x2f3136), delete_after=10)
            await message.author.send(embed=discord.Embed(description=f"hey {message.author.mention}, don't spam", color=0x2f3136).set_footer(text=message.guild.name, icon_url=message.guild.icon.url), delete_after=60)
            violations=self.too_many_spam.get_bucket(message)
            check=violations.update_rate_limit()
            if check:
                await message.author.timeout(datetime.timedelta(minutes=3))
                await message.author.send(embed=discord.Embed(description=f"You have been muted for spamming in {message.guild.name}"))
